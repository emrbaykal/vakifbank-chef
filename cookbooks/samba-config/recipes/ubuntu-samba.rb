...
# Copyright:: 2022, The Authors, All Rights Reserved.
  ...
  ruby_block "Fix passwd parameter to the /etc/nsswitch.conf file" do
      block do
        ...
        file.search_file_replace(/^passwd.*/, 'passwd:    files systemd winbind')
        file.write_file
      end
      not_if "grep -i '^passwd:\s*files\s*systemd\s*winbind$' /etc/nsswitch.conf"
  end

  ruby_block "Fix group parameter to the /etc/nsswitch.conf file" do
      block do
        file = Chef::Util::FileEdit.new(nsswitch_file)
        file.search_file_replace(/^group.*/, 'group:    files systemd winbind')
        file.write_file
      end
      not_if "grep -i '^group:\s*files\s*systemd\s*winbind$' /etc/nsswitch.conf"
  end
